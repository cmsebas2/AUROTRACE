...
<div class="max-w-7xl mx-auto py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-2xl font-black text-[#333333] tracking-tight">Módulo de Ejecución de Manufactura</h2>
            <p class="text-[#333333] font-medium opacity-80">Gestione y ejecute las fases de las órdenes activas en el piso de planta.</p>
        </div>
        @if(auth()->user()->hasPermission('crear_op'))
        <a href="{{ route('op.crear') }}" style="background-color: #0A2540;" class="inline-flex items-center px-6 py-3 text-white font-bold rounded-xl shadow-lg hover:opacity-90 transition-all">
                    ...
                    <tr class="hover:bg-slate-50 shadow-sm transition-colors border-l-4 border-transparent hover:border-aurofarma-blue">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center space-x-3">
                                ...
                                <a href="{{ $action['route'] }}" 
                                   title="Siguiente paso: {{ $action['next'] }}"
                                ...
                                <form action="{{ route('op.destroy', $op) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta OP?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <a href="{{ route('op.crear') }}" class="mt-6 text-aurofarma-blue font-black text-xs hover:underline uppercase tracking-widest">Crear mi primera OP →</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
