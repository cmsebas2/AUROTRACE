...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="card-3d p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-slate-200/80">
        <div class="flex items-center space-x-3">
            <div class="w-3 h-7 bg-gradient-to-b from-amber-500 to-orange-600 rounded-full shadow-[0_4px_12px_rgba(242,142,19,0.3)]"></div>
            <div>
                <h2 class="font-display text-2xl font-black text-slate-800 tracking-tight">Monitoreo de Órdenes Activas</h2>
                <p class="text-xs text-slate-500 font-medium">Control de trazabilidad en tiempo real para lotes en curso de planta</p>
            </div>
        </div>

        <a href="{{ route('batch.iniciar') }}" 
            ...
            <table class="min-w-full divide-y divide-slate-100 text-left">
                <thead>
                    <tr class="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white">
                        ...
                        <th class="px-6 py-4 text-[11px] font-black text-slate-200 uppercase tracking-wider">Estado / Despeje</th>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end space-x-2">
                                @php
                                    ...
                                    $label = 'CONTINUAR';
                                    $btnClass = 'bg-gradient-to-r from-[#005889] to-[#06B6D4] text-white shadow-3d-button hover:shadow-3d-cyan';

                                    if ($op->status === 'ACONDICIONAMIENTO') {
                                        $route = route('batch.despeje', $op->lote);
                                        $label = 'INICIAR ENVASE';
                                        $btnClass = 'bg-gradient-to-r from-[#028838] to-emerald-500 text-white shadow-[0_8px_16px_-4px_rgba(2,136,56,0.4)]';
                                    }
                                @endphp

                                <a href="{{ $route }}" 
                                   class="inline-flex items-center px-4 py-2 {{ $btnClass }} rounded-xl text-xs font-black uppercase tracking-wider transition-all transform hover:-translate-y-0.5 active:translate-y-0">
                                    <span>{{ $label }}</span>
                                ...
                                <form action="{{ route('ops.destroy', $op) }}" method="POST" onsubmit="return confirm('¿Confirma la eliminación definitiva de esta OP y sus registros bajo norma CFR 21?');">
                                    ...
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors border border-transparent hover:border-red-200" 
                                            title="Eliminar OP">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center text-slate-400">
                                ...
                                <h3 class="text-sm font-bold text-slate-700">Sin órdenes activas en línea</h3>
                                <p class="text-xs text-slate-400 mt-1">Todas las órdenes han sido completadas o no se han emitido nuevas.</p>
                                <a href="{{ route('batch.iniciar') }}" class="mt-4 text-cyan-600 font-black text-xs hover:underline uppercase tracking-widest">
                                    Crear nueva OP en Planta →
                                </a>
                            </div>
                        </td>
                    </tr>
