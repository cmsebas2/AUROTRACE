        ...
        <div class="w-64 h-screen overflow-y-auto bg-slate-900 text-white flex flex-col justify-between hidden md:flex shadow-2xl">
            <div>
                ...
                <nav class="flex-1 mt-6 px-4 pb-24 space-y-2 overflow-y-auto">
                    ...
                    <a href="{{ route('reconditioning.dashboard') }}" class="block py-3 px-4 rounded transition {{ request()->routeIs('reconditioning.dashboard') ? 'bg-white/10 border-l-4 border-aurofarma-blue text-white shadow-sm font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }}">
                        Inicio (Reacondicionamiento)
                    </a>
                    @endif
                    
                    @if(auth()->user()->hasPermission('ver_productos') && !auth()->user()->hasRole('Analista de Producción'))
                    ...
                    <details class="group [&_summary::-webkit-details-marker]:hidden" {{ request()->routeIs('reconditioning.*') ? 'open' : '' }}>
                        <summary class="flex items-center justify-between px-4 py-3 rounded transition cursor-pointer {{ request()->routeIs('reconditioning.*') ? 'bg-white/10 border-l-4 border-aurofarma-blue text-white shadow-sm font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }} list-none">
                            <div class="flex items-center">
                                <svg class="w-5 h-5 mr-3 text-aurofarma-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path></svg>
                                Reacondicionamiento
                            </div>
                        ...
                        <div class="mt-1 space-y-1 pl-4 border-l border-gray-700 ml-6">
                            <a href="{{ route('reconditioning.dashboard') }}" class="block py-2 px-4 rounded transition {{ request()->routeIs('reconditioning.dashboard') ? 'bg-gray-800 text-white font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }} text-sm">
                                Dashboard de Control
                            </a>
                            <a href="{{ route('reconditioning.create') }}" class="block py-2 px-4 rounded transition {{ request()->routeIs('reconditioning.create') ? 'bg-gray-800 text-white font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }} text-sm">
                                Entrada de Producto
                            </a>
                            <a href="{{ route('reconditioning.inventory') }}" class="block py-2 px-4 rounded transition {{ request()->routeIs('reconditioning.inventory') ? 'bg-gray-800 text-white font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }} text-sm">
                                Inventario de Bodega
                            </a>
                            <a href="{{ route('reconditioning.history') }}" class="block py-2 px-4 rounded transition {{ request()->routeIs('reconditioning.history') ? 'bg-gray-800 text-white font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }} text-sm">
                                Historial de Salidas
                            </a>
                            <a href="{{ route('reconditioning.planner') }}" class="block py-2 px-4 rounded transition {{ request()->routeIs('reconditioning.planner') ? 'bg-gray-800 text-white font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }} text-sm">
                    ...
                    @if(auth()->user()->hasPermission('ejecutar_manufactura') && !auth()->user()->hasRole('Analista de Producción'))
                    <a href="{{ route('op.ejecucion') }}" class="block py-3 px-4 rounded transition flex items-center {{ request()->routeIs('op.ejecucion') ? 'bg-white/10 border-l-4 border-aurofarma-teal text-white shadow-sm font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }}">
                    ...
                    @if(auth()->user()->hasPermission('ver_monitoreo_ops') && !auth()->user()->hasRole('Analista de Producción'))
                    <a href="{{ route('op.activas') }}" class="block py-3 px-4 rounded transition flex items-center {{ request()->routeIs('op.activas') ? 'bg-white/10 border-l-4 border-aurofarma-teal text-white shadow-sm font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }}">
                    ...
                    <!-- Módulo BATCH RECORDS (Expediente Maestro) -->
                    @if(!auth()->user()->hasRole('Analista de Producción'))
                    <a href="{{ route('batch-records.index') }}" class="block py-3 px-4 rounded transition flex items-center {{ request()->routeIs('batch-records.*') ? 'bg-white/10 border-l-4 border-aurofarma-teal text-white shadow-sm font-medium' : 'hover:bg-gray-800 hover:text-white text-gray-300' }}">
                        <svg class="w-5 h-5 mr-3 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        Batch Records
                    </a>
                    @endif

                    <!-- Calidad Menu -->
                    @if((auth()->user()->hasPermission('ver_aseguramiento_calidad') || auth()->user()->hasPermission('ver_formulas_maestras')) && !auth()->user()->hasRole('Analista de Producción'))
                    <details class="group [&_summary::-webkit-details-marker]:hidden">
                        ...
                        <div class="mt-1 space-y-1 pl-4 border-l border-gray-700 ml-6">
                            ...
                            <a href="{{ route('op.calidad') }}" class="block py-2 px-4 rounded transition hover:bg-gray-800 hover:text-white text-gray-300 text-sm">
